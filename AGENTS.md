...
- Optional quick lint check: run `node --check file.js` for syntax validation before reloading.
...
- Host permissions target `bbs.uestc.edu.cn` and rely on cookies for session detection; avoid broadening scopes without justification.
- Keep alarms lightweight (current 1-minute interval) and avoid blocking the service worker. For new settings, surface toggles in `options.html` and persist via `chrome.storage`.
